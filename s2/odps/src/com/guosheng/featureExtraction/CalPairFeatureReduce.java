...
public class CalPairFeatureReduce extends ReducerBase {
	...
	public void reduce(Record key, Iterator<Record> values, TaskContext context)
		...
		ArrayList<Long[]> trainFeature = getInitFeature(pairFeatureLength);
		ArrayList<Long[]> testFeature = getInitFeature(pairFeatureLength);
		...
		while (values.hasNext()) {
			...
			int index = getTrainRangeNum(dateStr, pairFeatureLength);
			if (index != -1) {
				trainFeature.get(index)[type]++;
			}
			index = getTestRangeNum(dateStr, pairFeatureLength);
			if (index != -1) {
				testFeature.get(index)[type]++;
			}

			if (dateStr.compareTo("07-22") < 0) { // train set
			...
			if (dateStr.compareTo("04-14") > 0) { // test set
...
//			double ratio = 123 / 98.0;
			...
			for(int i = 0; i < advFeatureTrain.length; ++i){
				resultTrain.set(4 * pairFeatureLength + 3 + i, (double)advFeatureTrain[i]); //8维高级特征乘以ratio
			}
		...
		if (!isFiltered(testFeature)) {
			...
			for(int i = 0; i < advFeatureTest.length; ++i){
				resultTest.set(4 * pairFeatureLength + 3 + i, (double)advFeatureTest[i]);
			}
	...
	private int getTestRangeNum(String dateStr, int pairFeatureLength) {
		// TODO Auto-generated method stub
		double timeWindow = 124 / (double)pairFeatureLength; // 2013-04-15至2013-08-16
		Calendar base = Calendar.getInstance();
		base.set(2013, 7, 16); // 设置Base为2013-08-16, Java的Calendar好违和
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		Date day;
		long dayDelta = 0;
		try {
	...
	private int getTrainRangeNum(String dateStr, int pairFeatureLength) {
		// TODO Auto-generated method stub
		double timeWindow = 99 / (double)pairFeatureLength; // 2013-04-15至2013-07-21
		Calendar base = Calendar.getInstance();
		base.set(2013, 6, 22); // 设置Base为2013-07-22, Java的Calendar好违和
		DateFormat df = new SimpleDateFormat("yyyy-MM-dd");
		Date day;
		long dayDelta = 0;
		try {
